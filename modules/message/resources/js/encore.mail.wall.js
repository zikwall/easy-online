encore.module('message.wall', function(module, require, $) {
   ...
   var ConversationView = Widget.extend();
    ...
    ConversationView.prototype.markSeen = function(id) {
        client.post(module.config.url.seen, {data: {id: id}}).then(function(response) {
            if(object.isDefined(response.messageCount)) {
                message.setMailMessageCount(response.messageCount);
            }
        }).catch(function(e) {
            module.log.error(e);
        });
    ...
    ConversationView.prototype.loadUpdate = function() {
        var  $lastEntry = this.$.find('.message-conversation-entry:last');
    ...
    ConversationView.prototype.loadMessage = function(evt) {
        ...
        client.get(this.options.loadMessageUrl, {data: {id:messageId}}).then(function(response) {
            ...
            $('#message-conversation-overview').find('.selected[data-message-preview]').find('.new-message-badge').hide();

            // Set new selection
            $('#message-conversation-overview').find('[data-message-preview]').removeClass('selected');
            $('#message-conversation-overview').find('[data-message-preview="'+messageId+'"]').addClass('selected').find('.new-message-badge').hide();
    ...
    ConversationView.prototype.updateSize = function() {
        ...
        var formHeight = $('.message-message-form').height();
    ...
    var getEntry = function(id) {
        return Widget.instance('.message-conversation-entry[data-entry-id="'+id+'"]');
    };

    var getRootView = function() {
        return Widget.instance('#message-conversation-root');
    }

    var init = function() {
       event.on('encore:modules:message:live:NewUserMessage', function (evt, events, update) {
           ...
           events.forEach(function(event) {
               ...
               message.setMailMessageCount(event.data.count);
           });

           //TODO: update notification count
       }).on('encore:modules:message:live:UserMessageDeleted', function (evt, events, update) {
           events.forEach(function(event) {
    ...
    var getOverViewEntry = function(id) {
        return $('#message-conversation-overview').find('[data-message-preview="'+id+'"]');
    };



    var loadMessage = function(evt) {
        if($('#message-conversation-root').length) {
